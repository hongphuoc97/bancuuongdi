 ...
 */
const Button = ({ href = '#', children, icon, variant = 'primary', fullWidth = false, ...rest }) => {
  ...
  const fullWidthClasses = fullWidth ? 'w-full justify-center' : '';

  return (
    <a href={href} className={`${baseClasses} ${variants[variant]} ${fullWidthClasses}`} {...rest}>
      {icon}
      {children}
    </a>
  );
