...
const BookingPopup = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex justify-center items-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 max-w-xl w-full transform transition-all duration-300">
          ...
          <form className="space-y-5">
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 rounded-lg p-1 bg-gray-100">
                <label>
                  <input type="radio" name="driverType" value="oto" className="peer sr-only" checked={driverType === 'oto'} onChange={(e) => setDriverType(e.target.value)} />
                  <div className="text-center py-2 px-3 rounded-md cursor-pointer text-gray-600 peer-checked:bg-white peer-checked:text-primary peer-checked:shadow font-semibold transition-all duration-200">Ô tô</div>
                </label>
                <label>
                  <input type="radio" name="driverType" value="xemay" className="peer sr-only" checked={driverType === 'xemay'} onChange={(e) => setDriverType(e.target.value)} />
                  <div className="text-center py-2 px-3 rounded-md cursor-pointer text-gray-600 peer-checked:bg-white peer-checked:text-primary peer-checked:shadow font-semibold transition-all duration-200">Xe máy</div>
                </label>
                <label>
            ...
            <div className="relative">
              ...
              <input type="time" id="pickupTime" defaultValue={defaultPickupTime} className="w-full pl-10 pr-3 py-2 border rounded-lg bg-gray-50" />
            </div>
            ...
            {showStopPoint && (
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute top-0 right-0 flex items-center gap-1 text-sm font-semibold text-red-600 hover:text-red-800 transition-colors"
                >
                  <MinusIcon />
                  Xóa
                </button>
            ...
            {!showStopPoint && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 text-primary font-semibold py-2 px-4 border-2 border-dashed border-primary/50 rounded-lg hover:bg-primary/10 transition-colors"
              >
                <PlusIcon />
                Thêm điểm dừng
              </button>
            ...
            <div className="bg-primary/10 p-4 rounded-lg flex justify-between items-center mt-6">
              <span className="font-bold text-gray-700">Ước tính chi phí:</span>
            ...
            <div className="flex justify-end gap-4 pt-4">
              <button
                type="button"
                onClick={onClose}
                className="bg-gray-200 text-gray-800 px-8 py-3 rounded-lg font-semibold hover:bg-primary-dark transition-colors shadow-md hover:shadow-lg"
              >
                Đóng
              </button>
              <button type="submit" className="bg-gray-200 text-gray-800 px-8 py-3 rounded-lg font-semibold hover:bg-primary-dark transition-colors shadow-md hover:shadow-lg">
                Xác nhận
              </button>
            </div>
          </form>
